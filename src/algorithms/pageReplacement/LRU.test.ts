...
describe("LRU Page Replacement Algorithm", () => {
	it("should replace least recently used pages", () => {
		const pages: Page[] = [
		...
		const updatedPages = LRU(pages, 3);
	...
	it("should handle RAM size larger than number of pages", () => {
		...
		const updatedPages = LRU(pages, 5);
	...
	it("should handle RAM size of 1", () => {
		const pages: Page[] = [
			{ id: 1, processId: 1, inMemory: false },
			{ id: 2, processId: 1, inMemory: false },
			{ id: 3, processId: 1, inMemory: false },
		...
		const updatedPages = LRU(pages, 1);

		expect(updatedPages).toHaveLength(5);
		expect(updatedPages[0].inMemory).toBe(true);
		expect(updatedPages[1].inMemory).toBe(false);
		expect(updatedPages[2].inMemory).toBe(true);
		expect(updatedPages[3].inMemory).toBe(false);
		expect(updatedPages[4].inMemory).toBe(true);
	});
