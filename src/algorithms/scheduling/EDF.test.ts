...
describe("EDF Scheduling Algorithm", () => {
	it("should schedule processes in order of earliest deadline", () => {
		...
		const scheduledProcesses = EDF(processes, 1, 0);
	...
	it("should handle processes with different arrival times", () => {
		...
		const scheduledProcesses = EDF(processes, 1, 1);
	...
	it("should consider overhead between processes", () => {
		...
		const scheduledProcesses = EDF(processes, 1, 2);
