...
export function App() {
  ...
  const [memoryState, setMemoryState] = useState<{ ram: Page[], disk: Page[] }>({ ram: [], disk: [] });

  const handleAddProcess = (process: Process) => {
    const newProcess = {
      ...process,
      id: processes.length + 1,
      pages: Array.from({ length: Math.min(process.pages.length, MAX_PAGES_PER_PROCESS) }, (_, index) => ({
        id: index + 1,
  ...
  return (
    <div>
      <h1>Simulador de Escalonamento de Processos e Gerenciamento de Memória</h1>
