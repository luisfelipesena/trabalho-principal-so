...
export function PageReplacement({ processes, ramSize, onMemoryUpdate }: PageReplacementProps) {
      ...
      <div>
        ...
        <ul>
          {memoryState.ram.map(page => (
            <li key={`${page.processId}-${page.id}`}>Process {page.processId}, Page {page.id}</li>
          ))}
        </ul>
      </div>
      <div>
