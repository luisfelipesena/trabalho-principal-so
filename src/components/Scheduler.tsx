...
export function Scheduler({ processes, quantum, overhead, onSchedule }: SchedulerProps) {
  const [algorithm, setAlgorithm] = useState<'FIFO' | 'SJF' | 'RoundRobin' | 'EDF'>('FIFO');
    ...
    <>
      ...
      <div>
        <label>
          ...
          <select
            value={algorithm}
            onChange={(e) => setAlgorithm(e.target.value as 'FIFO' | 'SJF' | 'RoundRobin' | 'EDF')}
          >
      ...
      <div>
        ...
        <ul>
          {scheduledProcesses.map((process) => (
            <li key={process.id}>
              Processo {process.id} - Tempo de chegada: {process.arrivalTime} - Tempo de execução: {process.executionTime} - Deadline: {process.deadline}
            </li>
          ))}
        </ul>
      </div>
